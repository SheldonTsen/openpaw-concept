...
**Goal**: Make the repo public and production-ready for external contributors.

### 9.1 Legal & License
- [x] Choose a license (MIT = permissive, Apache 2.0 = patent protection, GPL = copyleft)
- [x] Add `LICENSE` file to repo root
- [x] Add `license` field to `pyproject.toml`

### 9.2 Repo Hygiene (before going public)
- [x] Audit git history for secrets/credentials — `git log -p | grep -iE "key|secret|token|password"`
- [x] Confirm `.gitignore` covers `.env`, `neonize.db`, `data/`, `state/`, `workspace/`
- [x] Remove or scrub any internal references (personal phone numbers, internal URLs, private notes)
- [x] Review `CLAUDE.md` — strip internal-only notes if any

### 9.3 README Polish
- [x] Add badges: CI status, license, Python version
- [x] Add "Contributing" section (PR welcome, code style: ruff, tests required)
- [x] Add "License" section at the bottom
